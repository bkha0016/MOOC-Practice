...
; mutually recursive
;; good candidates for enscapulation:
;; 1. One function has one or more helpers closely linked to it
;; 2. The outside program really only wants to call the main function, not the helpers
...
(check-expect (find--all "F3" F1) false)
(check-expect (find--all "F3" F3) 3)
(check-expect (find--all "D4" D4) 0)
(check-expect (find--all "D6" D6) 0)
(check-expect (find--all "F3" D4) false)
(check-expect (find--all "F1" D4) 1)
(check-expect (find--all "F2" D4) 2)
(check-expect (find--all "F1" D6) 1)
(check-expect (find--all "F3" D6) 3)
