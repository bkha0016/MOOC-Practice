...
(define (pattern=? LOS1 LOS2)
  (cond
    [(empty? LOS2) true]
    [(and (empty? LOS1) (cons? LOS2)) false]
    [else
     (cond
       [(string=? (first LOS2) "A") (alphabetic? (first LOS1))]
       [(string=? (first LOS2) "N") (numeric? (first LOS1))]
       [else (pattern=? (rest LOS1) (rest LOS2))])]))

(pattern=? LOS1 PATTERN1)
(pattern=? LOS1 PATTERN2)
