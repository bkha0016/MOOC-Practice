...
;; Problem 1
;; produces analogous MUPL list with same elts
(define (racketlist->mupllist alst)
  (cond [(empty? alst) (aunit)]
        [else (apair (car alst) (racketlist->mupllist (cdr alst)))]))

;; produces analogous racket list with same elts
(define (mupllist->racketlist blst)
  (cond [(aunit? blst) empty]
        [else (cons (apair-e1 blst) (mupllist->racketlist (apair-e2 blst)))]))
